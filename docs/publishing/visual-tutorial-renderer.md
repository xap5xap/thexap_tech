...
- Absolute `https://www.thexap.com/...` RichText links render through their site-local path, including `/tools/opportunity-brief-builder`.
...
`npm run test:visual-tutorial` covers exact protocol parsing, legacy fallback, hero and inline caption consumption, attribution, PDF/DOCX rendering, native tool links, unsupported and missing Assets, raw HTML removal, and canonical/social metadata. Its CH-01-shaped fixture contains one hero, three inline graphics, PDF and DOCX downloads, and the native builder link.
