# Portfolio Positioning and Messaging
...
## Proof pillars and evidence plan

The portfolio may state the complete promise only when the portfolio as a whole provides credible evidence for every pillar. A single case study does not need equal depth in every pillar.

| Proof pillar | What the visitor should believe | Evidence available or identified | Evidence required before making a strong claim | Planned evidence owner |
| --- | --- | --- | --- | --- |
| Product strategy | Xavier can turn an opportunity into a clear audience, promise, scope, and product bet. | Armonía has an identifiable service, patient journey, public experience, and operational product that can be reconstructed as a real decision story. | Validate the original problem, Xavier's decisions, rejected directions, constraints, and publishable claims with Carla. | `XAP-49` |
| Brand and UX | Xavier can make the product understandable, credible, and emotionally appropriate for its audience. | The current Armonía experience, brand evolution, appointment journey, and cinematic direction are candidate public artifacts. Historical projects also document Figma and interface work. | Assemble before-and-after artifacts, explain the reasoning behind them, and obtain approval for every Armonía asset and claim. | `XAP-49` |
| Engineering and AI | Xavier can build a reliable product system and use AI for a meaningful job rather than decoration. | The current portfolio records nine years of full-stack delivery, production projects, Upwork evidence, and the architecture of this site. The strategy identifies the publishing engine as owned AI-system evidence. | Publish at least one case with understandable AI/system behavior, Xavier's ownership, failure handling, safeguards, and evaluation. Do not treat current homepage claims as proof. | `XAP-50`; later `XAP-55` |
| Launch | Xavier can translate a working product into a clear public story and reusable launch assets. | Armonía's public site and cinematic experience are candidate launch-story artifacts. | Show the launch narrative, asset set, decisions, intended audience, call to action, and what Xavier personally produced. Add a complete concept launch only after selection. | `XAP-49`; later `XAP-55` |
| Distribution | Xavier can choose a credible route to an audience and create assets for that route. | Armonía's appointment/content journey and the publishing engine are candidate evidence. | Name the channels, audience, hook, reusable assets, call to action, and observable signal. Do not claim reach or outcomes that have not been measured. | `XAP-49`, `XAP-50`; later `XAP-55` |
| Learning | Xavier can use audience response to make a product decision after launch. | The current public portfolio does not yet provide strong learning evidence. | Run one bounded distribution experiment, preserve the result, separate observation from interpretation, and document the next decision even if the result is weak. | `XAP-56` |

### Evidence labels

Case-study content should use these evidence states internally while drafting:

- **Observed:** directly inspectable behavior, artifact, or source.
- **Verified outcome:** a real result with a named source and scope.
- **Assumption:** a working belief that has not been validated.
- **Hypothesis:** a statement paired with a test and decision rule.
- **Illustrative:** simulated data or a demonstrative interface, labeled as such.
- **Planned:** future work that must not be written as current behavior.
...
- an initial discussion of the audience, opportunity, current product state, and desired change;
- an assessment of whether Xavier's product-partner model fits the work;
- a concrete next-step recommendation if there is a fit.

It does not promise free product strategy, a completed solution, or a guaranteed commercial outcome. The scheduling page should repeat enough of this context that the transition from CTA to Calendly feels intentional.

## Route-by-route copy direction

### Homepage `/`

**Role:** establish the complete promise and direct qualified visitors toward proof or a product conversation.
...
**Role:** explain why Xavier can credibly act as the product partner behind the promise.
...
**Role:** prove the positioning through a curated body of evidence.

Required changes:

- Replace the generic `Projects` introduction with `Selected products and systems` and an explanation of the portfolio thesis.
- Distinguish live/client work, owned products or experiments, and concept products explicitly.
- Lead with exactly three initial flagship positions and move credible historical engineering work into a secondary archive.
- For each flagship, state the audience, intended transformation, work type/status, Xavier's ownership, relevant product-cycle stages, and one honest proof point or hypothesis.
- Make the complete cycle visible and connect every stage to inspectable evidence.
- End with the contextual product-opportunity headline and supporting text above a `Schedule a meeting` button.

`XAP-47` owns the information architecture and taxonomy that implement this direction.

### Flagship case studies `/projects/[slug]`

**Role:** turn the positioning claim into a traceable product story.

Required changes:

- Lead with the transformation and audience rather than product name, role, responsibilities, and technologies.
- Explain the problem, insight or hypothesis, and why this product was the right bet.
- State Xavier's ownership across strategy, design, engineering, AI, media, launch, and distribution without implying work he did not perform.
- Show the key experience and system behavior with artifacts.
- Include decisions, constraints, trade-offs, rejected directions, and risks.
- Show how the product reached or is intended to reach people.
- Present real evidence where available; otherwise present an honest validation plan.
- End with learning and the next iteration.
- Make work type, current status, assumptions, illustrative data, and concept disclosure impossible to miss.
- Keep technologies as supporting implementation detail.

`XAP-48` owns the reusable case-study model. `XAP-49` applies it first to Armonía.

### Scheduling `/schedule-meeting`

**Role:** fulfill the expectation established by the primary CTA.

Required changes:

- Add a short explanation of who the conversation is for and what the visitor may bring.
- Repeat that an opportunity, problem, early concept, prototype, or existing product is enough to start.
- Avoid promising a free strategy deliverable or a specific outcome before fit and scope are understood.
